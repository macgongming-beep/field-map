#!/usr/bin/env node
// 일정·신청·배정 정책 묶음을 테스트 또는 운영 DB에 원자적으로 적용·복구한다.
...
const MIGRATION = 'supabase/migrations/20260907_1100_finalize_calendar_assignment_policies.sql'
const ROLLBACK = 'supabase/tools/_ROLLBACK_20260907_일정배정정책.sql'
const VERIFY = 'supabase/tools/_VERIFY_20260907_일정배정정책.sql'
const PSQL = process.env.PSQL_BIN ?? 'psql'
const TABLES = [
  'calendar_events', 'event_participants', 'event_card_assignments',
...
select json_build_object(
  ...
  'selects', (select count(distinct tablename) from pg_policies where schemaname='public'
    and tablename in (${tableList}) and cmd='SELECT'),
  'hidden_impls', (select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='public' and p.proname like '%\_impl\_20260907')
);
commit;`
...
console.log(`  정책          TEMP ${state.temp} · 최종 ${state.final} · SELECT 표 ${state.selects}/6`)
console.log(`  역할          ${JSON.stringify(state.roles)}`)
console.log(`  숨긴 구현     ${state.hidden_impls}/3`)

if (!((state.temp === 18 && state.final === 0 && state.hidden_impls === 0)
  || (state.temp === 0 && state.final === 18 && state.hidden_impls === 3))) {
  die(`예상하지 않은 상태입니다 (TEMP ${state.temp}, 최종 ${state.final}, 구현 ${state.hidden_impls})`)
}
if (state.selects !== 6) die('Realtime에 필요한 SELECT 정책이 빠졌습니다')
if (isTest && state.environment !== 'test') die('테스트 DB 표식이 없습니다')
if (!isTest && state.environment === 'test') die('운영 DB가 테스트로 표시돼 있습니다')

if (isRollback && state.temp === 18) { console.log('\n  이미 TEMP 상태입니다.\n'); process.exit(0) }
if (!isRollback && state.final === 18) {
...
if (!isTest) {
  ...
  if (arg('--confirm') !== PROD_REF) {
    console.log('\n  운영에는 적용하지 않았습니다.')
    console.log(`  적용: npm run apply:calendar-policies -- --confirm ${PROD_REF}`)
    console.log(`  복구: npm run apply:calendar-policies -- --rollback --confirm ${PROD_REF}\n`)
    process.exit(0)
  }
